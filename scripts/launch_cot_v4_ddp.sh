#!/usr/bin/env bash
# Fresh-from-base CoT finetune on the v4 dataset, GPUs 5,6 (DDP).
# Key differences vs the old v3 run:
#   - TRAIN_CSV = v4 (carries the `cot` column injected into the <think> block)
#   - MAX_SEQ_LEN=1024 (CoT targets reach ~880 tokens; 512 would truncate ~25%)
#   - INFERENCE_FINAL_ANSWER_PREFILL=0 -> eval runs in THINKING mode (matches the
#     official harness enable_thinking=True), no </think> prefill skip
#   - Scoring is already aligned to the official metric in train_sft.evaluate()
set -euo pipefail
